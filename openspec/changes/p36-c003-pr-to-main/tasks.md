...
- [ ] T1: Verify the premise — confirm sovereign SFU work is on `main` and the proof branch is gone:
...
- [ ] T2: Remove the dead `push:` trigger (`branches: ["sovereign-sfu-decode-proof"]`) from
...
- [ ] T3: Confirm the workflow still dispatches after the edit: `gh workflow list` shows
      "Decode proof" active, and `gh workflow run decode-proof.yml --ref main` is accepted.
- [ ] T4: Write `docs/PHASE-36-SIGNOFF.md` — record the gate decision from c002 (flip or hold), the
      run 29112243615 evidence (`ice=connected`, `framesDecoded=0`), G2/G3 as met, and G5 as retired
      with the reason.
- [ ] T5: Amend G5 in the phase `goals.md` to state the PR is moot: the work is already on `main`,
