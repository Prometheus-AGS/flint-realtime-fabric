...
async function main() {
    await connect(
        async (client: Client) => {
            ...
            //
            // !! BROKEN — DOUBLY UNSATISFIABLE. Flagged 2026-09-14 (p38-c005);
            // deliberately NOT fixed here, because repairing the pipeline is a
            // separate change from reconciling documentation.
            //
            //   1. It runs flutter_rust_bridge_codegen against
            //      crates/frf-ffi/src/lib.rs, a UniFFI crate. ADR-003 records that
            //      FRB's parser PANICS on `#[uniffi::export]` and that FRB and
            //      UniFFI cannot co-own the FFI crate. Dart is generated by
            //      `uniffi-bindgen-dart`, not FRB.
            //   2. Even if generate succeeded, the diff target
            //      sdks/dart/lib/src/rust/frb_generated.dart does not exist and is
            //      gitignored (.gitignore:67), so it can never exist in a clean
            //      checkout.
            //
            // Fixing this means replacing the stage with uniffi-bindgen-dart, or
            // deleting it. See ADR-003.
            //
